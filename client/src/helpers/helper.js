...
export async function paysavings({email, amount }){
  try {
    const response = await axios.post('/api/subscribe',{email, amount})
    const authorizationUrl = response.data.authorizationUrl
    window.location.href = `${authorizationUrl}&redirect_url=${window.location.origin}/dashboard`
  } catch (error) {
    console.log(error)
  }
}

export async function userBalance({ email }){
    try {
        const balance =  await axios.get('/api/generateOTP', { params: { email }})
